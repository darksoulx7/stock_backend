...
export class UserService {
  ...
  async verifyUser(email: string, emailOtp: string, phoneOtp: string): Promise<void> {
    try {
      // Step 1: Verify WhatsApp OTP
      const validateOtpp = await validateOtp(email, emailOtp, phoneOtp)
      if (!validateOtpp) {
        throw new Error("Incorrect otps");
      }

      // Step 2: Update user as verified in DynamoDB
